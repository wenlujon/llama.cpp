...
bool ggml_numa_migrate_active(void) {
    return g_state.numa.numa_strategy == GGML_NUMA_STRATEGY_MIGRATE;
}
...
static void ggml_compute_forward_mul_mat_one_chunk(
    ...
    const void * wdata = (src1->type == vec_dot_type) ? src1->data :
                         numa_migrate ? params->wdata_numa[node_id] : params->wdata;
#else
...
UseGgmlGemm1:;
    ...
    if (src1->type != vec_dot_type) {
        ...
        const bool numa_migrate = ggml_numa_migrate_active();
        const int node_id = numa_migrate ? ggml_get_node_from_cpu(ith) : 0;
        int round_cnts = numa_migrate ? ggml_cores_per_numa(ith) : nth;
        int start_id = numa_migrate ? ggml_get_start_id_in_node(ith) : ith;
        char * wdata = numa_migrate ? params->wdata_numa[node_id] : params->wdata;
#else
...
static void ggml_compute_forward_mul_mat_id(
    ...
    int round_cnts = numa_migrate ? ggml_cores_per_numa(ith) : nth;
    int start_id = numa_migrate ? ggml_get_start_id_in_node(ith) : ith;
    void * wdata_base = numa_migrate ? params->wdata_numa[node_id] : params->wdata;
#else
    void * wdata_base = params->wdata;
    int start_id = ith;
    int round_cnts = nth;
#endif
    assert(round_cnts);
    void * wdata_cur = wdata_base;
    ...
    // IQP currently reads params->wdata directly, so keep it on the standard path.
    ...
    GGML_ASSERT(params->wsize >= (size_t)((char *) wdata_cur - (char *) wdata_base));


    if (src1->type != vec_dot_type) {
        char * wdata = (char *) wdata_base;
...
#ifdef GGML_USE_NUMA_MIGRATE
    for (int node = 0; node < GGML_NUMA_MIGRATE_NODES; node++) {
        numa_free(threadpool->n_barrier_node[node], sizeof(atomic_int));
        numa_free(threadpool->n_barrier_passed_node[node], sizeof(atomic_int));
    }
    for (int i = 0; i < GGML_BARRIER_NODE_CNTS; i++) {
        free(threadpool->n_barrier_passed_last[i]);
    }
#endif
...
#ifdef GGML_USE_NUMA_MIGRATE
            ggml_barrier_numa_aware(state->threadpool, state->ith, graph_barrier++ % GGML_BARRIER_NODE_LAST);
#else
            ggml_barrier(tp);
#endif
        }
